...
def recurrence2d(z,z_target,w,n):
    ...
    M_target = np.zeros(shape=(n,n,z_target.size),dtype=np.complex128)
    ...
    for j in range(0,n):
        for k in range(0,n):
                ...
                sub_p_target = np.array(P_target[k,j,:])
                M[k,j,:] = np.array(P[k,j,:])
                M_target[k,j,:] = np.array(P_target[k,j,:])
                P = P - dot2x2(w,sub_p,P)*sub_p
                P_target = P_target - dot2x2(w_target,sub_p_target,P_target)*sub_p_target
                P = P/norm2x2(w,P)
