...
def recurrence2d(z,z_target,w, data, size, limit):
    ...
    for k in large:
        ...
        for j in range(l,k+1):
            ...
            dot_data = dot2x2(w,P[k-j+l,j,:],P*V,1)
            #dot_target = dot2x2(w,P[k-j+l,j,:],P*V,1,z_target.size)
            
            P = P - dot_data*P[k-j+l,j,:]
            P_target = P_target - dot_data*P_target[k-j+l,j,:]
    ...
    for k in large:
        ...
        for j in range(l,k+1):
            if k==0 and j==0:
                ...
                dot_data = dot2x2(w,D,P*V,0)
                #dot_target = dot2x2(w,D,P*V,0,z_target.size)
                
                P = P - dot_data*D
                P_target = P_target - dot_data*D_target
    ...
    return P_target, Ig_aux, std_a, s, idx
...
P, Ig, std_a, S, idx_max = recurrence2d(z.flatten(),z_target.flatten(), w.flatten(), img2, N, limit)
print("Orden de polinomio al cuadrado es: ", S, "y el polinomio que da menor desviación estándar es: ", idx_max)
